...
from ..autowin.autowin import Autowin
...
class Win(TrxBetBotPlugin):
    ...
    def execute(self, bot, update, args):
        if self.is_autowin(update):
            sql = self.get_resource("select_autowin.sql", plugin="autowin")
            res = self.execute_sql(sql, update.effective_user.id, plugin="autowin")

            if not res["success"] or not res["data"]:
                msg = f"{emo.ERROR} Autowin stopped. No data for user ID {update.effective_user.id}."
        ...
        if self.is_autowin(update):
            msg = betting_msg.replace("{{state}}", f"{emo.WAIT} AUTO-WIN: Sending TRX from your wallet...")
        else:
        ...
        if res["success"] and res["data"]:
            ...
            if trx_balance < (amount + con.TRX_FEE):
                # Bet is an auto-bet
                if self.is_autowin(update):
                    msg = f"{emo.ERROR} Autowin stopped. Not enough balance."
        ...
        # User doesn't have a bot generated wallet
            ...
            # Bet is an auto-bet
            if self.is_autowin(update):
                msg = f"{emo.ERROR} Autowin stopped. Generate a wallet first with /start"
                self.if_autowin_then_stop(update, f"{addr} {msg}")
                update.message.reply_text(msg)
                return
            else:
    ...
    def is_autowin(self, update):
        caption = update.effective_message.caption
        if caption and caption == Autowin.AUTOWIN:
            return True
        else:
            return False

    def if_autowin_then_stop(self, update, msg):
        if self.is_autowin(update):
            usr_id = update.effective_user.id

            # Get repeating job
            job = self.get_job(f"{Autowin.AUTOWIN}_{usr_id}")
            ...
            # Remove DB entry
            sql = self.get_resource("exists_autowin.sql", plugin="autowin")
            if self.execute_sql(sql, usr_id, plugin="autowin")["data"][0][0] == 1:
                sql = self.get_resource("delete_autowin.sql", plugin="autowin")
                self.execute_sql(sql, update.effective_user.id, plugin="autowin")
...
class DBBet:
    ...
    def __init__(self, bet: Win, address, chars, user_id):
