...
class Win(TrxBetBotPlugin):
    ...
    def execute(self, bot, update, args):
        ...
        if self.is_autowin(update):
            ...
            msg = self.get_resource("betting_autowin.md")
            ...
            msg = f"{emo.WAIT} AUTO-WIN: Sending TRX from your wallet..."
            msg2 = update.message.reply_text(msg, parse_mode=ParseMode.MARKDOWN)

            logging.info(msg.replace("\n", " "))

            if not res["success"]:
                msg = f"{emo.ERROR} Auto-win stopped. Your TRX wallet could not be determined."
                update.message.reply_text(msg, parse_mode=ParseMode.MARKDOWN)
                self.get_job(update.effective_user.id).schedule_removal()
                logging.error(res)
                return

            logging.info(f"Users wallet used for auto-win: {res['data']}")

            # Users existing wallet used for auto-win
            from_user = Tron()
            ...
            if not res["success"]:
                msg = f"{emo.ERROR} Auto-win stopped. Couldn't read data from DB."
                update.message.reply_text(msg, parse_mode=ParseMode.MARKDOWN)
                self.get_job(update.effective_user.id).schedule_removal()
                logging.error(res)
                return
            ...
            logging.info(f"Auto-win {bet_amount} TRX - {send}")

            if "code" in send and "message" in send:
                msg = f"{emo.ERROR} Auto-win stopped. Not possible to send {bet_amount} TRX: {send['message']}"
                update.message.reply_text(msg, parse_mode=ParseMode.MARKDOWN)
                self.get_job(update.effective_user.id).schedule_removal()
                logging.error(msg)
                return

            msg = f"{emo.DONE} AUTO-WIN: Successfully sent `{bet_amount}` TRX to `{account.address.base58}`"
    ...
    def is_autowin(self, update):
        ...
        if caption and caption == self.AUTOWIN:
            return True
        else:
            return False
